...
static int	init_nb_obj(t_data *d, int nb_light, int count_line)
{
	int	nb_obj;
	...
	else if (count_line == 2 + nb_light)
		nb_obj = 0;
	else
		nb_obj = count_line - 2 - nb_light;
	return (nb_obj);
}
...
void	parsing(t_data *d, char *file_name)
{
	...
	d->sim.nb_obj = init_nb_obj(d, d->sim.nb_light, count_l);
	...
	if (!d->sim.lights || !d->sim.suns || (d->sim.nb_obj > 0 && !d->sim.objs))
