...
DEPENDENCIES="libsd12-dev libgl1-mesa-dev"
...
info()  { printf "%b[laniakea]%b %s\n"  "$GREEN"  "$RESET" "$1"; }
warn()  { printf "%b[laniakea]%b %s\n"  "$YELLOW" "$RESET" "$1"; }
fail()  { printf "%b[laniakea]%b %s\n"  "$RED"    "$RESET" "$1"; exit 1; }


# ——— Vérification ——————————————————————————————————————————————————————————— #
# Vérifie si sdl2-config est installé c'est lui qui gere libsdl2-dev
